...
calculator op num = parseOp op <*> readMaybe num
  where
...
boolOrInt s = (Left <$> validateBool s) <|> (Right <$> validateInt s)
  where
    validateBool s = case readMaybe s of
      Just b  -> pure b
      Nothing -> invalid "Not a Bool"
    validateInt s = case readMaybe s of
      Just i  -> pure i
...
normalizePhone s =
  let stripped = filter (not . isSpace) s
      tooLong = if length stripped > 10 then invalid "Too long" else pure stripped
      checkChars = traverse (\c -> if isDigit c then pure c else invalid ("Invalid character: " ++ [c])) stripped
  in tooLong *> checkChars
...
parseExpression s =
  case words s of
    [a, op, b] -> parseOp op <*> parseArg a <*> parseArg b
  ...
  where
    ...
    parseArg s = case readMaybe s of
      ...
      Nothing -> invalid ("Invalid " ++ if all isDigit s then "variable" else "number" ++ ": " ++ s)
