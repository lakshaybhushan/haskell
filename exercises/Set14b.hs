...
balance db account = do
  xs <- query db balanceQuery (Only account)
  return $ sum (map fromOnly xs)
------------------------------------------------------------------------------
...
parseCommand [cmd, account, amount]
  | cmd == T.pack "deposit" = Deposit account <$> parseInt amount
  | cmd == T.pack "withdraw" = Withdraw account . negate <$> parseInt amount
...
perform conn (Just (Withdraw account amount)) = do
  deposit conn account (-amount)  -- Withdraw is just deposit with a negative amount
  return $ T.pack "OK"
