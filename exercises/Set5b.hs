...
allValues f (Node value left right) = f value && allValues f left && allValues f right
...
isOrdered (Node value left right) = allValues (< value) left && allValues (> value) right && isOrdered left && isOrdered right
...
set _ _ Empty = Empty
set (StepL:steps) value (Node v left right) = Node v (set steps value left) right
set (StepR:steps) value (Node v left right) = Node v left (set steps value right)
...
search value tree = search' [] tree
  where
    search' steps Empty = Nothing
    search' steps (Node v left right)
      | v == value = Just steps
      | otherwise = case search' (steps ++ [StepL]) left of
                      Just path -> Just path
                      Nothing -> case search' (steps ++ [StepR]) right of
                        Just path -> Just path
                        Nothing -> Nothing