...
interpreterHelper x y (cmd:cmds)
  | cmd == "up" = interpreterHelper x (y + 1) cmds
  | cmd == "down" = interpreterHelper x (y - 1) cmds
  | cmd == "left" = interpreterHelper (x - 1) y cmds
  | cmd == "right" = interpreterHelper (x + 1) y cmds
  | cmd == "printX" = show x : interpreterHelper x y cmds
  | cmd == "printY" = show y : interpreterHelper x y cmds
  | otherwise = interpreterHelper x y cmds
