...
paint (Picture pattern) (Shape shape) (Picture base) =
    Picture (\coord -> if shape coord then pattern coord else base coord)
...
averageColor colors = Color (avg getRed) (avg getGreen) (avg getBlue)
  where
    avg f = sum (map f colors) `div` length colors

instance Transform Blur where
  apply Blur (Picture p) = Picture (\coord@(Coord x y) ->
    averageColor [p (Coord x' y') | (x', y') <- neighbors coord])
    where
      neighbors (Coord x y) = [(x, y), (x-1, y), (x+1, y), (x, y-1), (x, y+1)]
