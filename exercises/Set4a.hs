...
transfer from to amount bank = if Map.notMember from bank || Map.notMember to bank || amount < 0 || Map.findWithDefault 0 from bank < amount
  then bank
  else Map.adjust (subtract amount) from $ Map.adjust (+ amount) to bank
